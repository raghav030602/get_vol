...
//	long long strike_gap=50; // Strike gap in Rs
	...
	string spot_symbol = "NIFTYMIDSELECT";
	unordered_map<string , long long >mp_spread;
	if(symbol==spot_symbol){
        	mp_spread[spot_symbol]=ltp;
	}

    	//string time= get_curr_time();
	string curr_date = get_curr_date();
	std::string filename = "MIDCPNIFTY_spot.csv";
	...
	double actual_spread=ltp-mp_spread[spot_symbol];
...
//	long long strike_gap_2=50;	
	...
	//temp*=10000;
	if(underlying[2]=="FUT"){
		if(mp_spread.find(spot_symbol)==mp_spread.end()){
		...
		mp_option[symbol]=ltp - spread_value1*100;
		long long temp=mp_option[symbol];
//		cout<<"Temp = "<<temp<<endl;
